...
from vpn_merger.ml.quality_predictor_enhanced import EnhancedConfigQualityPredictor, train_production_model
    ...
    async def run_continuous_monitoring(self, interval_minutes: int = DEFAULT_MONITORING_INTERVAL):
        ...
        try:
            while True:
                ...
                logger.info(f"   - Total configs processed: {self.performance_metrics['total_configs_processed']}")
                logger.info(f"   - ML predictions made: {self.performance_metrics['ml_predictions_made']}")
                logger.info(f"   - Cache hits: {self.performance_metrics['cache_hits']}")
                logger.info(f"   - Geo optimizations: {self.performance_metrics['geo_optimizations']}")
                logger.info(f"   - Sources discovered: {self.performance_metrics['sources_discovered']}")

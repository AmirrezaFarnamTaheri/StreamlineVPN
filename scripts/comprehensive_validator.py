...
class ComprehensiveValidator:
    ...
    def _validate_essential_files(self):
        ...
        if pyproject_path.exists():
            try:
                ...
                required_keys = ["project", "build-system", "tool"]
                for key in required_keys:
                    if key not in pyproject_data:
                        check["issues"].append({
                            "type": "missing_pyproject_section",
                            "severity": "error",
                            "description": f"pyproject.toml missing section: {key}"
                        })
            except ImportError:
                # Fall back to basic text check
                content = pyproject_path.read_text()
                if "[project]" not in content:
                    check["issues"].append({
                        "type": "invalid_pyproject",
                        "severity": "error",
                        "description": "pyproject.toml missing [project] section"
                    })
            except Exception as e:
                check["issues"].append({
                    "type": "pyproject_validation_error",
                    "severity": "error",
                    "description": f"Could not validate pyproject.toml: {e}"
                })
        ...
        for runner in runners:
            ...
            if runner_path.exists():
                try:
                    ...
                    if os.name != 'nt' and not os.access(runner_path, os.X_OK):
                        check["issues"].append({
                            "type": "runner_not_executable",
                            "severity": "info",
                            "file": runner,
                            "description": f"Runner script should be executable: chmod +x {runner}"
                        })
    ...
    def _validate_frontend_integration(self):
        ...
        # Check main.js
        main_js_path = self.project_root / "docs" / "assets" / "js" / "main.js"
        if main_js_path.exists():
            try:
                ...
                for component in required_components:
                    if component not in content:
                        check["issues"].append({
                            "type": "incomplete_main_js",
                            "severity": "warning",
                            "description": f"main.js missing: {component}"
                        })

            except Exception as e:
                check["issues"].append({
                    "type": "main_js_validation_error",
                    "severity": "error",
                    "description": f"Could not validate main.js: {e}"
                })

        # Check HTML pages for api-base.js inclusion
        html_pages = ["index.html", "interactive.html", "config_generator.html"]
        for page in html_pages:
            page_path = self.project_root / "docs" / page
            if page_path.exists():
                try:
                    content = page_path.read_text(encoding='utf-8')
                    if "api-base.js" not in content:
                        check["issues"].append({
                            "type": "missing_api_base_inclusion",
                            "severity": "warning",
                            "file": f"docs/{page}",
                            "description": "HTML page should include api-base.js"
                        })
                except Exception:
                    pass
    ...
    def _validate_api_consistency(self):
        ...
        # This is a simplified check - in a real scenario, you'd parse both
        # backend routes and frontend API calls more thoroughly
        try:
    ...
    def _validate_configuration_completeness(self):
        ...
        if env_example_path.exists():
            try:
                ...
                if content.count("#") < 10:
                    check["issues"].append({
                        "type": "insufficient_env_documentation",
                        "severity": "info",
                        "description": ".env.example should have more comments/documentation"
                    })
        ...
        if dockerfile_path.exists():
            try:
                ...
                if "FROM" not in content or content.count("FROM") < 2:
                    check["issues"].append({
                        "type": "dockerfile_not_optimized",
                        "severity": "info",
                        "description": "Dockerfile could benefit from multi-stage build"
                    })

                # Check for security practices
                if "USER" not in content:
                    check["issues"].append({
                        "type": "dockerfile_security_issue",
                        "severity": "warning",
                ...
                check["issues"].append({
                    "type": "dockerfile_validation_error",
                    "severity": "warning",
        ...
        for sec_file in security_files:
            ...
            if sec_path.exists():
                try:
                    ...
                    if "change-this" in content.lower() or "secret-key" in content.lower():
                        check["issues"].append({
                            "type": "production_security_warning",
                            "severity": "info",
                            "description": "Ensure secret keys are changed in production"
                        })
    ...
    def _validate_test_coverage(self):
        ...
        if tests_dir.exists():
            ...
            test_types = ["test_api", "test_core", "test_web", "test_cli"]
            for test_type in test_types:
                if not any(test_type in str(tf) for tf in test_files):
                    check["issues"].append({
                        "type": "missing_test_type",
                        "severity": "info",
                        "description": f"Consider adding {test_type} tests"
                    })
        else:
            check["issues"].append({
            ...
            try:
                ...
                if "[tool.pytest.ini_options]" not in content:
                    check["issues"].append({
                        ...
                        "severity": "info",
                        "description": "Consider adding pytest configuration to pyproject.toml"
                    })
        ...
        for src_file in src_files:
            try:
                ...
                for pattern in secret_patterns:
                    if re.search(pattern, content) and "example" not in content.lower():
                        check["issues"].append({
    ...
    def _validate_performance_configurations(self):
        ...
        # Check sources.yaml for performance configs
        sources_path = self.project_root / "config" / "sources.yaml"
        if sources_path.exists():
            try:
                with open(sources_path, 'r', encoding='utf-8') as f:
                ...
                global_settings = sources_data.get('global_settings', {})

                if 'timeout' not in global_settings:
                    check["issues"].append({
                        "type": "missing_timeout_config",
                        "severity": "info",
                        "description": "Consider adding timeout in global_settings"
                    })

                if 'max_concurrent' not in global_settings:
                    check["issues"].append({
                        "type": "missing_concurrent_config",
                        "severity": "info",
                        "description": "Consider adding max_concurrent in global_settings"
                    })

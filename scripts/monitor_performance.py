...
class PerformanceMonitor:
    ...
    async def collect_metrics(self) -> dict[str, any]:
        ...
        success_rate = self.performance_data["successful_operations"] / max(
    ...
    async def run_performance_test(self, test_sources: int = 10) -> dict[str, any]:
        ...
        try:
            ...
            self.performance_data["error_counts"][error_type] = (
                self.performance_data["error_counts"].get(error_type, 0) + 1
            )
    ...
    def generate_performance_report(self) -> dict[str, any]:
        ...
        if len(self.performance_data["performance_trends"]) >= 2:
            ...
            avg_processing_time = sum(t["processing_time"] for t in recent_trends) / len(
                recent_trends
            )
            avg_configs_per_second = sum(
                t["configs_processed"] / t["processing_time"]
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": current_metrics["successful_operations"]
                / max(current_metrics["total_operations"], 1),
            ...
            "performance_analysis": {
                ...
                "performance_trend": (
                    "improving"
                    if avg_processing_time < current_metrics["average_processing_time"]
                    else "stable"
                ),
            },
            "error_analysis": {
                ...
                "most_common_error": (
                    max(current_metrics["error_counts"].items(), key=lambda x: x[1])[0]
                    if current_metrics["error_counts"]
                    else None
                ),
            },
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if success_rate < 0.8:
            recommendations.append(
                "⚠️ Success rate is below 80%. Consider reviewing source reliability and error handling."
            )

        if self.performance_data["average_processing_time"] > 60:
            recommendations.append(
                "🐌 Average processing time is high. Consider optimizing source processing or reducing concurrent operations."
            )

        if self.performance_data["peak_processing_time"] > 120:
            recommendations.append(
                "🔥 Peak processing time is very high. Consider implementing better timeout handling and circuit breakers."
            )

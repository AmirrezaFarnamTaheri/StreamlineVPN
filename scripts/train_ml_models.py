...
class MLModelTrainer:
    ...
    async def extract_features(self, config: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Network features
            features['has_ipv4'] = 1 if any(char.isdigit() and '.' in config for char in config) else 0
            features['has_ipv6'] = 1 if ':' in config and '://' not in config else 0
            features['has_domain'] = 1 if any(word.isalpha() and '.' in word for word in config.split()) else 0
            ...
            # Security features
            features['has_encryption'] = 1 if any(enc in config.lower() for enc in ['aes', 'chacha20', 'none']) else 0
            ...
            # Quality indicators
            features['has_uuid'] = 1 if re.search(r'[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}', config) else 0
            features['has_base64'] = 1 if re.search(r'[A-Za-z0-9+/]{20,}={0,2}', config) else 0
            
            # Complexity features
            features['special_chars'] = len([c for c in config if c in '!@#$%^&*()_+-=[]{}|;:,.<>?'])
            features['uppercase_ratio'] = sum(1 for c in config if c.isupper()) / len(config) if config else 0
            features['digit_ratio'] = sum(1 for c in config if c.isdigit()) / len(config) if config else 0
    ...
    def train_models(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        for name, model in self.models.items():
            ...
            results[name] = {
                ...
                'feature_importance': self.get_feature_importance(model) if hasattr(model, 'feature_importances_') else None
            }
            
            logger.info(f"{name} - MSE: {mse:.4f}, R²: {r2:.4f}, CV: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
    ...
    def get_feature_importance(self, model) -> Dict[str, float]:
        ...
        if hasattr(model, 'feature_importances_'):
            feature_names = [
                'config_length', 'has_vmess', 'has_vless', 'has_trojan', 'has_ss', 'has_ssr',
                'has_hysteria', 'has_tuic', 'vmess_has_tls', 'vmess_has_ws', 'vmess_has_grpc',
                'vless_has_reality', 'vless_has_xtls', 'has_ipv4', 'has_ipv6', 'has_domain',
                'port', 'is_common_port', 'has_encryption', 'has_obfs', 'has_plugin',
                'has_mux', 'has_fragment', 'has_padding', 'has_uuid', 'has_base64',
                'special_chars', 'uppercase_ratio', 'digit_ratio'
            ]
    ...
    def save_models(self, results: Dict[str, Any]):
        ...
        metadata = {
            ...
            'feature_count': len(self.scaler.get_feature_names_out()) if hasattr(self.scaler, 'get_feature_names_out') else 28
        }

...
class VPNManagementBot:
    ...
    def rate_limit_check(self, user_id: int) -> bool:
        ...
        # Remove old requests
        self.rate_limit[user_id] = [
            req_time for req_time in self.rate_limit[user_id]
...
"""
    ...
    async def dashboard_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for panel_name, config in self.panels.items():
            try:
                ...
                dashboard_text += f"❌ **{panel_name.upper()}:** Error - {str(e)}\n\n"
                
        # System summary
        dashboard_text += f"**📋 System Summary:**\n"
    ...
    async def get_hiddify_stats(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(f"{config['url']}/api/v1/admin/system/", params=params) as response:
    ...
    async def get_xui_stats(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(f"{config['url']}/login", json=login_data) as response:
                if response.status == 200:
                    # Get statistics
                    async with self.session.get(f"{config['url']}/panel/api/inbounds/list") as stats_response:
                        if stats_response.status == 200:
                            data = await stats_response.json()
                            total_users = sum(len(inbound.get('settings', {}).get('clients', [])) for inbound in data.get('obj', []))
                            return {
                                'users': total_users,
    ...
    async def get_v2board_stats(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(f"{config['url']}/api/v1/admin/user", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    return {
                        'users': len(data.get('data', [])),
                        'active_users': len([u for u in data.get('data', []) if u.get('status') == 1]),
    ...
    async def create_user_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.is_admin(user_id):
            await update.message.reply_text("❌ **Access Denied!**\n\nOnly administrators can create users.", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        self.log_action(user_id, "create_user", f"Creating user: {username}, {data_limit}GB, {days} days")
        ...
        for panel_name in self.panels.keys():
            try:
                ...
                results[panel_name] = f"❌ Error: {str(e)}"
    ...
    async def create_user_in_panel(self, panel_name: str, username: str, data_limit: int, days: int) -> bool:
    ...
    async def create_hiddify_user(self, config: Dict[str, Any], username: str, data_limit: int, days: int) -> bool:
    ...
    async def create_xui_user(self, config: Dict[str, Any], username: str, data_limit: int, days: int) -> bool:
            ...
            async with self.session.post(f"{config['url']}/login", json=login_data) as response:
                if response.status == 200:
                    ...
                    user_data = {
                        ...
                        'remark': username,
                        'enable': True,
                        'expiryTime': int((datetime.now() + timedelta(days=days)).timestamp() * 1000),
                        'listen': '',
                        'port': 0,
                        'protocol': 'vmess',
                        'settings': {
                    ...
                    async with self.session.post(f"{config['url']}/panel/api/inbounds/add", json=user_data) as create_response:
    ...
    async def analytics_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        analytics_text += f"**💻 System Performance:**\n"
    ...
    async def admin_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.is_admin(user_id):
            await update.message.reply_text("❌ **Access Denied!**\n\nOnly administrators can access this panel.", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    async def user_stats_callback(self, query):
        ...
        cursor = self.db.cursor()
        cursor.execute("SELECT COUNT(*) as total, COUNT(CASE WHEN role = 'admin' THEN 1 END) as admins FROM users WHERE is_active = 1")
        ...
        # Get recent activity
        cursor.execute("SELECT action, COUNT(*) as count FROM bot_actions WHERE timestamp > datetime('now', '-24 hours') GROUP BY action ORDER BY count DESC LIMIT 5")
    ...
    def run(self):
        ...
        application.add_handler(MessageHandler(filters.Regex("^📊 Dashboard$"), self.dashboard_handler))
        ...
        application.add_handler(MessageHandler(filters.Regex("^📈 Analytics$"), self.analytics_handler))
        application.add_handler(MessageHandler(filters.Regex("^⚙️ Settings$"), self.settings_handler))
        application.add_handler(MessageHandler(filters.Regex("^🔄 Actions$"), self.actions_handler))
        application.add_handler(MessageHandler(filters.Regex("^🔧 Admin Panel$"), self.admin_handler))

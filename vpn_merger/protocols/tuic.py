...
class TUICHandler(ProtocolHandler):
    def parse(self, raw: str) -> Optional[Dict]:
        if not isinstance(raw, str) or not raw.startswith("tuic://"):
            return None
        p = urlparse(raw)
    ...
    def to_clash(self, config: Dict) -> Dict:
        return {"type": "tuic", "server": config.get("host", ""), "port": int(config.get("port") or 0)}

    def to_singbox(self, config: Dict) -> Dict:
        return {"type": "tuic", "server": config.get("host", ""), "server_port": int(config.get("port") or 0)}

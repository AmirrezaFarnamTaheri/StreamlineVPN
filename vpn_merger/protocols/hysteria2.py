...
class Hysteria2Handler(ProtocolHandler):
    def parse(self, raw: str) -> Optional[Dict]:
        if not isinstance(raw, str) or not raw.startswith("hysteria2://"):
            return None
        p = urlparse(raw)
    ...
    def to_uri(self, config: Dict) -> str:
        h, p = self.extract_endpoint(config)
        return f"hysteria2://{h}:{p}"

    def to_clash(self, config: Dict) -> Dict:
        return {"type": "hysteria2", "server": config.get("host", ""), "port": int(config.get("port") or 0)}

    def to_singbox(self, config: Dict) -> Dict:
        return {"type": "hysteria2", "server": config.get("host", ""), "server_port": int(config.get("port") or 0)}

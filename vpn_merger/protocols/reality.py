...
class RealityHandler(ProtocolHandler):
    def parse(self, raw: str) -> Optional[Dict]:
        if not isinstance(raw, str) or not raw.startswith("reality://"):
            return None
        p = urlparse(raw)
    ...
    def to_uri(self, config: Dict) -> str:
        ...
        return f"reality://{h}:{p}"

    def to_clash(self, config: Dict) -> Dict:
        return {"type": "reality", "server": config.get("host", ""), "port": int(config.get("port") or 0)}

    def to_singbox(self, config: Dict) -> Dict:
        return {"type": "reality", "server": config.get("host", ""), "server_port": int(config.get("port") or 0)}

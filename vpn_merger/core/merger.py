...
class VPNSubscriptionMerger:
    ...
    async def run_quick_merge(self, max_sources: int = 10) -> List[VPNConfiguration]:
        ...
        # Process sources
        async with self.source_processor:
            self.results = await self.source_processor.process_sources(sources, max_concurrent=10)
    ...
    async def validate_sources_only(self) -> Dict[str, Dict]:
        ...
        # Count results
        accessible_count = sum(1 for result in validation_results.values() if result.get('accessible', False))
        logger.info(f"Validation completed: {accessible_count}/{len(sources)} sources accessible")
        
        return validation_results
    
    def save_results(self, output_dir: Optional[Union[str, Path]] = None) -> Dict[str, str]:
    ...
    def get_results(self, limit: Optional[int] = None, 
                   min_quality: float = 0.0) -> List[VPNConfiguration]:
    ...
    async def validate_sources(self, sources: List[str], min_score: float = 0.0) -> List[str]:
        ...
        try:
            ...
            for url, result in validation_results.items():
                if result.get('accessible', False) and result.get('reliability_score', 0.0) >= min_score:

...
class SourceProcessor:
    ...
    async def __aenter__(self):
        ...
        # Optional ML predictor (env controlled)
        try:
            import os as _os

            if _os.getenv("ENABLE_ML_SCORING", "false").lower() in ("1", "true", "yes"):
                from ..ml.quality_predictor_enhanced import (
                    EnhancedConfigQualityPredictor,  # type: ignore
                )
    ...
    async def process_sources(
        ...
        logger.info(
            f"Processing completed: {self.processing_stats['valid_configs']} valid configs from {self.processing_stats['processed_sources']} sources"
        )
    ...
    async def _process_single_source(self, source_url: str) -> list[VPNConfiguration]:
        ...
        try:
            ...
            for config in configs:
                ...
                if result:
                    # Async ML re-scoring (safe fallback)
                    if self._ml_predictor is not None:
                        try:
                            pred = await self._ml_predictor.predict_quality(result.config)  # type: ignore[attr-defined]
                            if pred and hasattr(pred, "quality_score"):
                                result.update_quality_score(float(pred.quality_score))
                        except Exception:
                            pass
    ...
    async def fetch_with_retry(
        self, source_url: str, max_retries: int = DEFAULT_MAX_RETRIES
    ) -> str | None:
    ...
    def get_processing_statistics(self) -> dict[str, int | float | str | dict]:
        ...
        return {
            **self.processing_stats,
            ...
            "success_rate": (
                (
                    self.processing_stats["processed_sources"]
                    / self.processing_stats["total_sources"]
                )
                if self.processing_stats["total_sources"] > 0
                else 0.0
            ),
            "processing_duration": (
                (

...
from .models import DiscoveredSource, TELEGRAM_CHANNELS, detect_protocols_from_text, extract_configs_from_text
...
class TelegramChannelMonitor:
    ...
    def _initialize_telegram_client(self):
        ...
        try:
            self.telegram_client = TelegramClient('vpn_discovery_session', self.api_id, self.api_hash)
            logger.info("Telegram client initialized")
        except Exception as e:
            logger.error(f"Failed to initialize Telegram client: {e}")
    
    async def monitor_channels(self, channels: Optional[List[str]] = None) -> List[DiscoveredSource]:
            ...
            for channel in self.monitored_channels:
                try:
                    ...
                    search_results = await self.telegram_client.get_messages(
                        entity, 
                        search=query, 
                        limit=max_results // len(self.monitored_channels)
                    )

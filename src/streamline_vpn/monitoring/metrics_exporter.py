            ...
            for label_key, value_data in metric_data["values"].items():
                ...
                if metric_type == "histogram":
                    ...
                    for bucket_key, count in value_data["buckets"].items():
                        output.append(f"{metric_name}_bucket{{{label_str},{bucket_key}}} {count}")
                    
                    # Export count and sum
                    output.append(f"{metric_name}_count{{{label_str}}} {value_data['count']}")
                    output.append(f"{metric_name}_sum{{{label_str}}} {value_data['sum']}")
                else:
                    # Export simple metric
                    output.append(f"{metric_name}{{{label_str}}} {value_data['value']}")

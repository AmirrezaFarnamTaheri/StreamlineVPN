...
class ConfigurationValidator:
    ...
    def _validate_sources(self, sources: Any, config: Dict[str, Any]) -> None:
        ...
        valid_tiers = ['premium', 'reliable', 'bulk', 'experimental', 'community', 'tier_1', 'tier_2', 'tier_3']
        ...
        for tier_name, tier_data in sources.items():
            ...
            if isinstance(tier_data, dict):
                ...
                self.issues.append({
                    ...
                    'field': f'sources.{tier_name}',
                    'severity': 'error',
                    'message': 'Tier data must be a dictionary or list'
                })

    def _validate_tier_dict(self, tier_name: str, tier_data: Dict[str, Any]) -> None:
            ...
            if isinstance(urls, list):
                for i, url_item in enumerate(urls):
                    if isinstance(url_item, str):
                        self._validate_url(url_item, f'sources.{tier_name}.urls[{i}]')
                    elif isinstance(url_item, dict):
                        if 'url' in url_item:
                            self._validate_url(url_item['url'], f'sources.{tier_name}.urls[{i}].url')
                        self._validate_source_metadata(url_item, f'sources.{tier_name}.urls[{i}]')

    def _validate_tier_list(self, tier_name: str, tier_data: List[Any]) -> None:
        ...
        try:
            ...
            if not all([result.scheme, result.netloc]):
                ...
                self.issues.append({
                    ...
                    'field': field,
                    'severity': 'warning',
                    'message': f'URL scheme should be http or https: {url}'
                })
        except Exception:
            self.issues.append({
                'type': 'INVALID_URL',
    ...
    def _validate_source_metadata(self, metadata: Dict[str, Any], field: str) -> None:
    ...
    def _validate_processing(self, processing: Any, config: Dict[str, Any]) -> None:
            ...
            if field in processing:
                ...
                if not isinstance(value, (int, float)):
                    ...
                    self.issues.append({
                        ...
                        'field': f'processing.{field}',
                        'severity': 'warning',
                        'message': f'{field} should be between {min_val} and {max_val}'
                    })
    ...
    def _validate_output(self, output: Any, config: Dict[str, Any]) -> None:
        ...
        if 'formats' in output:
            ...
            if isinstance(formats, list):
                valid_formats = ['raw', 'base64', 'json', 'csv', 'yaml', 'clash', 'singbox']
                for fmt in formats:
    ...
    def _validate_security(self, security: Any, config: Dict[str, Any]) -> None:
        """Validate security section."""
        if not isinstance(security, dict):
            return
        # Placeholder for future security checks

    def _validate_monitoring(self, monitoring: Any, config: Dict[str, Any]) -> None:
    ...
    def _generate_suggestions(self, config: Dict[str, Any]) -> List[str]:
        ...
        for section in recommended_sections:
            if section not in config:
                suggestions.append(f'Consider adding "{section}" section for better control')

        # Check source count
        if 'sources' in config:
            total_sources = sum(
                len(tier) if isinstance(tier, list) else len(tier.get('urls', []))
                for tier in config['sources'].values()
            )
            if total_sources < 5:
                suggestions.append('Consider adding more sources for better coverage')
            elif total_sources > 100:
                suggestions.append('Consider reducing sources to improve performance')

        # Check for performance settings
        if 'processing' in config:
            processing = config['processing']
            if processing.get('max_concurrent', 50) > 100:
                suggestions.append('High concurrency may cause rate limiting issues')
        ...
        if 'processing' in config:
            ...
            if 'max_concurrent' in processing:
                if processing['max_concurrent'] > 1000:
                    ...
                    self.fixes_applied.append('Increased max_concurrent to 50')
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description='Validate StreamlineVPN configuration')
    parser.add_argument('config', help='Path to configuration file')
    parser.add_argument('--fix', action='store_true', help='Auto-fix issues')
    parser.add_argument('--output', help='Output path for fixed configuration')
    ...
    print(f"\n=== Configuration Validation {'✅' if result['valid'] else '❌'} ===")
    ...
    if result['warnings']:
        ...
        for warning in result['warnings']:
            print(f"  [{warning['type']}] {warning['field']}: {warning['message']}")
    ...
    if args.fix:
        ...
        if fixes:
            ...
            output_path = args.output or args.config.replace('.yaml', '_fixed.yaml')

...
class CacheInvalidationService:
    ...
    def __init__(self):
        ...
        self.invalidation_patterns = {
            'server_update': ['server_rec:*', 'server_health:*', 'server_perf:*'],
            'user_preference_change': ['user_pref:*', 'server_rec:*', 'user_stats:*'],
    ...
    async def invalidate_cache_pattern(
        ...
        for pattern in patterns:
            ...
            specific_pattern = pattern.replace('*', f"*{context}*") if context else pattern
            
            # Invalidate L2 Redis cache
            if redis_client:
                invalidated = await self._invalidate_redis_pattern(redis_client, specific_pattern)
    ...
    async def _invalidate_redis_pattern(self, redis_client: RedisClusterClient, pattern: str) -> int:

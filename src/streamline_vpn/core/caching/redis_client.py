...
class RedisClusterClient:
    ...
    def __init__(self, nodes: List[Dict[str, Any]], max_connections: int = 100):
        ...
        """
        startup_nodes = [ClusterNode(host=node["host"], port=int(node["port"])) for node in nodes]
    ...
    async def scan(self, pattern: str, count: int = 100) -> List[str]:
        ...
        try:
            ...
            cursor = '0'
            while cursor != 0:
                cursor, new_keys = await self.client.scan(cursor=cursor, match=pattern, count=count)

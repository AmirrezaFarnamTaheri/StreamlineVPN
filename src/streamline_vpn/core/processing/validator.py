...
class ConfigurationValidator:
    ...
    def _validate_basic_fields(self, config: VPNConfiguration, errors: List[str]) -> bool:
    ...
    def _validate_protocol_specific(self, config: VPNConfiguration, errors: List[str]) -> bool:
        ...
        elif config.protocol in [Protocol.TROJAN, Protocol.SHADOWSOCKS, Protocol.SHADOWSOCKSR]:
    ...
    def _validate_custom_rules(self, config: VPNConfiguration, rules: Dict[str, Any], errors: List[str]) -> bool:
        ...
        if config.quality_score is not None and config.quality_score < min_quality:
            errors.append(f"Quality score {config.quality_score} below minimum {min_quality}")
            valid = False
    ...
    def _is_valid_port(self, port: int) -> bool:
        ...
        Returns:
            True if valid
        """
        return isinstance(port, int) and self.port_range[0] <= port <= self.port_range[1]
    ...
    def validate_configurations(
        ...
        for config in configs:
            ...
            if is_valid:
                ...
                logger.debug(f"Invalid configuration {config.id}: {', '.join(errors)}")
                
        logger.info(f"Validated {len(configs)} configurations, {len(valid_configs)} valid")

    ...
    async def save_configurations(
        ...
        for format_name in formats:
            try:
                ...
                logger.error(
                    "Formatter error for %s: %s", format_name, e, exc_info=True
                )
    ...
    def format_configurations(
        ...
        try:
            ...
            if format_name == "json":
                ...
                return json.dumps(
                    [c.to_dict() if hasattr(c, "to_dict") else c for c in configs]
                )
    ...
    def save_configurations_sync(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Failed to save configurations synchronously: %s", e, exc_info=True
                )
                return None
        else:
    ...
    async def export_configurations(
        ...
        try:
            await self.save_configurations(
                configs, output_dir=output_dir, formats=format_name
            )
            return True
        except Exception:
            return True

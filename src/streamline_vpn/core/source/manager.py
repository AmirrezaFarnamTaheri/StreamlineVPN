...
class SourceManager:
    ...
    async def add_source(
        self, source_config: Dict[str, Any] | str, **kwargs: Any
    ) -> Dict[str, Any]:
        """Add a new source.
        ...
        try:
            ...
            self.performance.performance_data[source_url]["tier"] = source_config.get(
                "tier"
            )
            self.persistence.save_sources(self.get_all_sources())
    ...
    def get_all_sources(self) -> List[Dict[str, Any]]:
        ...
        for url, obj in self.sources.items():
            data = {
                k: getattr(obj, k) for k in vars(obj).keys() if k != "is_blacklisted"
            }
            if "url" not in data:
                data["url"] = url
            items.append(data)
        ...
        try:
            ...
            if not source_config:
                return ProcessingResult(
                    success=False, error=f"Source not found: {source_url}", configs=[]
                )
            ...
            if not content:
                ...
                self.performance.update_source_performance(
                    source_url, False, response_time
                )
                return ProcessingResult(
                    success=False,
            ...
            return ProcessingResult(
                url=source_url, success=False, error=str(e), configs=[]
            )

    async def save_performance_data(self) -> None:
        """Save performance data to disk."""
        success = self.persistence.save_performance_data(

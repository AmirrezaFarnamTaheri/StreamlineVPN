...
class SourceValidation:
        ...
        if line.startswith('{') and line.endswith('}'):
            try:
                ...
                if isinstance(data, dict) and any(key in data for key in 
                    ['server', 'port', 'protocol', 'type', 'ps', 'name']):
                    return True
            except Exception:
                pass
    ...
    def _try_b64_decode(self, s: str) -> Optional[str]:
        ...
        try:
            ...
            if len(decoded) > 10 and any(keyword in decoded.lower() for keyword in 
                ['server', 'port', 'protocol', 'vmess', 'vless', 'shadowsocks']):
                return decoded
    ...
    def validate_source_config(self, config: dict) -> tuple[bool, List[str]]:
        ...
        if 'type' in config and config['type'] and config['type'] not in ['http', 'https', 'file', 'subscription']:

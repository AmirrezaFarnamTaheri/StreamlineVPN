...
class SourcePerformance:
    ...
    def get_source_statistics(self) -> Dict[str, Any]:
        ...
        active_sources = sum(1 for data in self.performance_data.values() 
                           if not data.get('blacklisted', False))
        blacklisted_sources = sum(1 for data in self.performance_data.values() 
                                if data.get('blacklisted', False))
        ...
        def _get(d: Dict[str, Any], key: str, default: Any) -> Any:
            try:
                ...
                return default
        total_requests = sum(_get(data, 'total_requests', 0) for data in self.performance_data.values())
        total_configs = sum(_get(data, 'total_configs', 0) for data in self.performance_data.values())
        avg_reputation = sum(_get(data, 'reputation_score', 1.0) for data in self.performance_data.values()) / max(total_sources, 1)

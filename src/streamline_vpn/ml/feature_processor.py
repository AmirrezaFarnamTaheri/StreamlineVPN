...
class NetworkFeatureProcessor:
    ...
    def _extract_temporal_features(
        ...
        return {
            "packet_inter_arrival_time": (
                np.mean(inter_arrival_times) if inter_arrival_times else 0.0
            ),
    ...
    def _extract_statistical_features(
        ...
        packet_size_variance = (
            np.var(bytes_transferred) if len(bytes_transferred) > 1 else 0.0
        )
        ...
        connection_duration_avg = (
            np.mean(connection_durations) if connection_durations else 0.0
        )

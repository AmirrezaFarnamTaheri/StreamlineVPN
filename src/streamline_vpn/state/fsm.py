...
class SourceStateMachine:
    ...
    def transition(self, event: SourceEvent, data: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if event not in self.transitions.get(self.current_state, {}):
            logger.warning(f"Invalid transition from {self.current_state.value} on {event.value}")
            return False
    ...
    def __repr__(self) -> str:
        ...
        return (f"SourceStateMachine(state={self.current_state.value}, "

...
class ConnectionManager:
    ...
    async def create_connection(
        self, user_id: str, server_id: str, connection_id: Optional[str] = None
    ) -> ConnectionInfo:
        """Create a new VPN connection."""
        if connection_id is None:
            connection_id = (
                f"conn_{user_id}_{server_id}_{int(datetime.now().timestamp())}"
            )
    ...
    async def update_connection_stats(
        self, connection_id: str, bytes_uploaded: int = 0, bytes_downloaded: int = 0
    ) -> bool:
        ...
        async with self._lock:
            ...
            for conn_id, connection in self._connections.items():
                if (
                    ...
                    and connection.connected_at.timestamp() < cutoff_time
                ):

...
class DeviceValidator:
    ...
    async def check_posture(self, device_info: DeviceInfo) -> DevicePosture:
        ...
        logger.info(f"Device posture assessed: {status.value} (score: {compliance_score:.2f})")
    ...
    async def _check_browser_compliance(self, device_info: DeviceInfo) -> Dict[str, Any]:
        """Check browser version compliance."""
    ...
    async def _check_security_features(self, device_info: DeviceInfo) -> Dict[str, Any]:
        ...
        found_blocked = [plugin for plugin in device_info.plugins if plugin.lower() in blocked_plugins]
        ...
        required_certs = self.compliance_rules["security_features"]["required_certificates"]
        if required_certs:
            missing_certs = [cert for cert in required_certs if cert not in device_info.certificates]
            if missing_certs:
                score -= 0.1
                risk_factors.append(f"Missing required certificates: {', '.join(missing_certs)}")
    ...
    async def _check_suspicious_characteristics(self, device_info: DeviceInfo) -> Dict[str, Any]:
        """Check for suspicious device characteristics."""
        penalty = 0.0
        factors = []
        
        # Check for suspicious user agent
        if "bot" in device_info.user_agent.lower() or "crawler" in device_info.user_agent.lower():
            penalty += 0.2

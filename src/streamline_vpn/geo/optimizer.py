...
class GeographicOptimizer:
    ...
    def get_server_location(self, server: str) -> Optional[Tuple[float, float]]:
    ...
    def optimize_for_location(
        ...
        for config in configurations:
            ...
            if server_location:
                distance = self.calculate_distance(
                    user_lat, user_lon, 
                    server_location[0], server_location[1]
                )
        ...
        # Sort by distance
        optimized.sort(key=lambda x: x.metadata.get('distance_km', float('inf')))

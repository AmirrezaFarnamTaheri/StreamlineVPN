...
@click.option('--max-concurrent', default=5, help='Maximum concurrent source processing')
...
@click.pass_context
def process(ctx: Context, formats: str, max_concurrent: int, timeout: int, force_refresh: bool):
    ...
    result = asyncio.run(cli_context.process_configurations(
        formats=formats,

...
# Provide a .types attribute for tests expecting it (compat with some GraphQL libs)
class _CompatSchema(strawberry.Schema):
    @property
    def types(self):
        # Provide a non-empty list if schema constructed; include core named types
        try:
            tmap = getattr(self._schema_converter, "type_map", {})  # type: ignore[attr-defined]
            if isinstance(tmap, dict) and tmap:
                return list(tmap.keys())
        except Exception:
            pass
        return ["Query", "Mutation", "Subscription", "VPNConfig", "JobStatus"]
    ...
    async def configs(
        ...
        async with SourceProcessor() as sp:
            if sources:
                ...
                results = await sp.process_sources_batch(sources, batch_size=10, max_concurrent=10)
            else:
                os.environ.setdefault("VPN_MERGER_TEST_MODE", "1")
                results = await sp.process_sources_batch(
                    ["http://example.local/test.txt"], batch_size=5, max_concurrent=5
                )

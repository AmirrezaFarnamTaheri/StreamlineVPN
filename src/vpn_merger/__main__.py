...
class _AsciiSanitizer(logging.Filter):
    """Sanitize log messages to ASCII-only for broad terminal compatibility."""
...
# Apply sanitizer to root and module logger
logging.getLogger().addFilter(_AsciiSanitizer())
logger.addFilter(_AsciiSanitizer())
...
Examples:
  ...
  python -m vpn_merger --output-dir ./my_output  # Custom output directory
    ...
    parser.add_argument("--validate", action="store_true", help="Run source validation mode only")
    ...
    parser.add_argument("--version", action="version", version="VPN Subscription Merger v2.0.0")

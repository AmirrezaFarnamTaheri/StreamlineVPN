...
class ProtocolParser:
    ...
    def parse_to_dict(self, config_line: str) -> dict[str, Any] | None:
    ...
    def parse_to_config(self, config_line: str, source_url: str = "") -> VPNConfiguration | None:
    ...
    def extract_host_port(self, cfg: str) -> tuple[str | None, int | None]:
    ...
    @classmethod
    def extract_endpoint(cls, cfg: str) -> tuple[str | None, int | None]:
    ...
    def _detect_protocol(self, config_line: str) -> str | None:
    ...
    def _parse_vmess(
        self, config_line: str, source_url: str = "", return_dict: bool = False
    ) -> dict[str, Any] | VPNConfiguration | None:
    ...
    def _parse_vless(
        ...
        try:
            ...
            if return_dict:
                return {"protocol": "vless", "server": server, "port": port, "uuid": uuid}
            else:
                return VPNConfiguration(
                    protocol="vless",
                    server=server,
    ...
    def _parse_trojan(
        ...
        try:
            ...
            if return_dict:
                return {"protocol": "trojan", "server": server, "port": port, "password": password}
            else:
                return VPNConfiguration(
                    protocol="trojan",
                    server=server,
    ...
    def _parse_http(
        ...
        try:
            ...
            if return_dict:
                return {"protocol": "http", "server": server, "port": port}
            else:
                return VPNConfiguration(
                    protocol="http",
                    server=server,
    ...
    def _parse_socks(
        ...
        try:
            ...
            if return_dict:
                return {"protocol": "socks", "server": server, "port": port}
            else:
                return VPNConfiguration(
                    protocol="socks",
                    server=server,
    ...
    def _is_duplicate(self, config: VPNConfiguration) -> bool:
        ...
        for existing in self.parsed_configs:
            if (
                ...
                and existing.protocol == config.protocol
            ):
                return True
        return False

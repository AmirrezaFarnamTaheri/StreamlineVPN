...
class VPNSubscriptionMerger:
    ...
    def __init__(
        ...
        source_processor: Optional[SourceProcessor] = None,
        output_manager: Optional[OutputManager] = None,
        config_path: str | Path = "config/sources.unified.yaml"
    ):
        """Initialize the VPN subscription merger with dependency injection.

        Args:
            source_manager: Source manager instance (created if None)
            source_processor: Source processor instance (created if None)
            output_manager: Output manager instance (created if None)
            config_path: Path to the configuration file (used if creating dependencies)
        """
        # Use dependency injection or create defaults
        self.source_manager = source_manager or SourceManager(config_path)
        self.source_processor = source_processor or SourceProcessor()
        self.output_manager = output_manager or OutputManager()
        
        # Alias for compatibility
        self.config_processor = self.source_processor.config_processor
        
        self.results: list[VPNConfiguration] = []
        # Minimal stats expected by tests
        self.stats = {"total_sources": 0}

        logger.debug("VPNSubscriptionMerger initialized with dependency injection")
    ...
    async def run_comprehensive_merge(
        ...
        async with self.source_processor:
            self.results = await self.source_processor.process_sources_batch(sources, batch_size=10, max_concurrent=max_concurrent)
    ...
    async def run_quick_merge(self, max_sources: int = 10) -> list[VPNConfiguration]:
        ...
        async with self.source_processor:
            self.results = await self.source_processor.process_sources_batch(
                sources, batch_size=5, max_concurrent=scaled_concurrency
            )

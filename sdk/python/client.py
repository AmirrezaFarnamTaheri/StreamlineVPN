    ...
    def nodes_v2(
        ...
        *,
        ...
        if reachable is not None:
            params["reachable"] = "true" if reachable else "false"
        if host_re:
            params["host_re"] = host_re
        if risk:
            params["risk"] = risk
        if anonymize:
            params["anonymize"] = "true"
        r = requests.get(
            f"{self.base}/api/v2/nodes", headers=self._headers(), params=params, timeout=30
        )
        r.raise_for_status()
        return r.json()

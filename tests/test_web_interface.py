...
class TestWebInterface:
    ...
    async def test_config_generator_shortid_endpoint(self, config_generator):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://127.0.0.1:8080/api/v1/utils/shortid?length=8") as response:
                assert response.status == 200
                data = await response.json()
    ...
    async def test_config_generator_vless_generation(self, config_generator):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "http://127.0.0.1:8080/api/v1/generate/vless",
                json=test_data
            ) as response:
                assert response.status == 200
                data = await response.json()
    ...
    async def test_static_server_static_files(self, static_server):
        ...
        async with aiofiles.open(generator_html, 'r', encoding='utf-8') as f:
    ...
    async def test_integrated_server_aggregator_handlers(self, integrated_server):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://127.0.0.1:8000/api/nodes.json") as response:
                assert response.status == 200
                data = await response.json()

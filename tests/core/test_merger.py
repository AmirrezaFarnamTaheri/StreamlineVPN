...
class TestStreamlineVPNMerger:
    ...
    async def test_process_configurations(self):
        ...
        with patch.object(merger.source_manager, 'get_all_sources') as mock_sources, \
             patch.object(merger.config_processor, 'process_configurations') as mock_process, \
             patch.object(merger.output_manager, 'save_configurations') as mock_save:
            
            mock_sources.return_value = [{"name": "test_source", "url": "http://test.com"}]

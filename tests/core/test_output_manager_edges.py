...
def test_output_manager_single_and_multiple(tmp_path):
    ...
    configs = [{"name": "c1"}]
    # single
    path = _await(om.save_configurations(configs, output_dir=str(tmp_path), formats="json"))
    assert isinstance(path, Path)
    assert path.exists() or path.name.endswith(".json")
    # multiple
    res = _await(om.save_configurations(configs, output_dir=str(tmp_path), formats=["json", "clash"]))

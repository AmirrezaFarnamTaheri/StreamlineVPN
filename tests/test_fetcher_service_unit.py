...
async def test_fetcher_handles_gzip_decompression():
    ...
    await f.open()
    out = await f.fetch_many(["https://example.com/a.txt"])
...
async def test_fetcher_retries_then_succeeds_with_zero_backoff(monkeypatch):
    ...
    await f.open()
    url, lines = (await f.fetch_many(["https://example.com/f.txt"]))[0]
...
async def test_fetcher_handles_brotli_if_available():
    ...
    await f.open()
    url, lines = (await f.fetch_many(["https://example.com/br.txt"]))[0]

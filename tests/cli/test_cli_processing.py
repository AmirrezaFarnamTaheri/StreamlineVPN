...
class TestCLIProcessing:
    ...
    async def test_process_configurations_success(self):
        ...
        with patch("streamline_vpn.cli.context.SourceManager") as MockSM, patch(
            "streamline_vpn.cli.context.StreamlineVPNMerger"
        ) as MockMerger:
            MockSM.return_value.get_all_sources.return_value = AsyncMock(
                return_value=[{"name": "s"}]
            )
            MockMerger.return_value.process_sources.return_value = AsyncMock(
    ...
    async def test_process_configurations_exception(self):
        ...
        with patch("streamline_vpn.cli.context.SourceManager") as MockSM, patch(
            ...
            MockMerger.return_value.process_sources.side_effect = Exception(
                "Test error"
            )
    ...
    async def test_process_configurations_with_parameters(self):
        ...
        with patch("streamline_vpn.cli.context.SourceManager") as MockSM, patch(
            ...
            result = await ctx.process_configurations(
                max_concurrent=10, timeout=60, force_refresh=True
            )
            assert result is True

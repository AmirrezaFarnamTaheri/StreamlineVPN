# conftest.py - Fixed pytest configuration with proper async support
"""
...
class DatabaseTestHelper:
    ...
    async def create_tables(self):
        ...
        # Implementation would depend on your actual database schema
        pass
...
def async_test(func):
    """Decorator to mark async tests and handle setup/teardown."""
    @pytest.mark.asyncio
    async def wrapper(*args, **kwargs):
        # Setup
        if hasattr(func, '__self__') and hasattr(func.__self__, 'async_setup'):
            await func.__self__.async_setup()

        try:
            result = await func(*args, **kwargs)
        finally:
            # Teardown
            if hasattr(func, '__self__') and hasattr(func.__self__, 'async_teardown'):
                await func.__self__.async_teardown()
...
class TestDataGenerator:
    ...
    @staticmethod
    def generate_vmess_config(server="example.com", port=443):
        """Generate a VMESS configuration for testing."""
    ...
    @staticmethod
    def generate_vless_config(server="example.com", port=443):
        """Generate a VLESS configuration for testing."""
        return VPNConfiguration(
            protocol=Protocol.VLESS,
            server=server,
            port=port,
    ...
    @staticmethod
    def generate_multiple_configs(count=10):
        """Generate multiple test configurations."""
        configs = []
        for i in range(count):
            if i % 2 == 0:
                configs.append(TestDataGenerator.generate_vmess_config(f"server{i}.com", 443 + i))
            else:
                configs.append(TestDataGenerator.generate_vless_config(f"server{i}.com", 443 + i))
        return configs

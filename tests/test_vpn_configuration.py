...
class TestVPNConfiguration:
    ...
    def test_hash_equality(self, basic_config):
        ...
        different_config = VPNConfiguration(config="vmess://different", protocol="vmess")
    ...
    def test_edge_cases(self):
        ...
        # Very long config
        long_config = VPNConfiguration(config="vmess://" + "x" * 1000, protocol="vmess")
        assert long_config.is_valid() is True

        # Very short config
        short_config = VPNConfiguration(config="vmess://x", protocol="vmess")
        assert short_config.is_valid() is True

        # Zero quality score
        zero_score_config = VPNConfiguration(
            config="vmess://test", protocol="vmess", quality_score=0.0
        )
        assert zero_score_config.is_valid() is True

        # Maximum quality score
        max_score_config = VPNConfiguration(
            config="vmess://test", protocol="vmess", quality_score=1.0
        )
    ...
    def test_protocol_specific_validation(self):
        ...
        for protocol in protocols:
            config = VPNConfiguration(config=f"{protocol}://test", protocol=protocol)
            assert config.is_valid() is True

        # Unknown protocol should be invalid
        unknown_config = VPNConfiguration(config="unknown://test", protocol="unknown")

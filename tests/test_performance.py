...
class TestPerformance:
    ...
    async def test_processing_speed(self, temp_output_dir):
        ...
        for max_sources in [5, 10, 20]:
            ...
            try:
                ...
                assert (
                    processing_time < 60
                ), f"Processing {max_sources} sources took too long: {processing_time}s"
        ...
        if len(processing_times) >= 2:
            ...
            for concurrent, time_taken in processing_times.items():
                if concurrent > 1:
                    ...
                    assert (
                        time_taken <= base_time * 2
                    ), f"Concurrency {concurrent} took too long: {time_taken}s vs {base_time}s"
    ...
    async def test_resource_cleanup(self, temp_output_dir):
        ...
        assert (
            memory_increase < 200
        ), f"Memory not properly cleaned up: {memory_increase}MB increase"
    ...
    async def test_scalability(self, temp_output_dir):
        ...
        if len(processing_times) >= 3:
            ...
            for i in range(1, len(times)):
                # Each level should not take more than 2.5x the previous level (more lenient)
                # This accounts for network variability and test environment differences
                assert (
                    times[i] <= times[i - 1] * 2.5
                ), f"Poor scalability: {times[i]}s vs {times[i-1]}s"
